...
if __name__ == '__main__':
 ...
 outFile=open("./alpha_for_participants_no_norm.txt", 'w') #w for truncating
...
#########################################################################
# norm_par=[0.0,1.0,2.0,10.0,20.0,50.0,100.0,200.0,300.0,500.0,1000.0,10000.0]

 stable_voxels=[25,50,100,125,150,225,250,300,400,500,600,700,800]
 acc_for_vox=np.zeros((9,13))
 for parts in no_parts:
  ...
  #      result.append(item)
     ...
     fmri_data_proc=np.zeros((58,s_v))
     fmri_data_final=np.zeros((58,s_v))
     for x in range(0,58):
     ...
     test_data1=test_data1.reshape((s_v,1))
     test_data2=test_data2.reshape((s_v,1))
     #print(test_data1.shape)
     ...
     mle_est=np.zeros((s_v,latent_dims+1))#zeros 25
  ...
  #          #for y in range(500):#every voxel
       model = linear_model.Ridge(300,fit_intercept=True,normalize=False)#####Ridge(alpha=0.5)
  ...
  #    #print(bias.shape)
       bias=np.reshape(np.array(bias),(s_v,1))
  ...
  #   p2=model.predict(sf2)
    ...
    acc_for_vox[int(parts)-1,int(index_s)]=float(accuracy)
