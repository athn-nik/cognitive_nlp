...
for part in ["M01" ,"M02", "M03", "M04" ,"M05", "M06" ,"M07", "M08" ,"M09" ,"M10", "M13" ,"M14" ,"M15" ,"M16" ,"M17" ,"P01"]:
    ...
    for wt in weights_lst:
        ...
        real_low = [x for x in real if x<0.25]
        real_low_index = [real.index(x) for x in real if x<0.25]
        real_high = [x for x in real if x>0.75]
        real_high_index = [real.index(x) for x in real if x>0.75]
